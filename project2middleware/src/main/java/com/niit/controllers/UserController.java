...
public ResponseEntity<?> userRegistration(@RequestBody User user){
	...
	if(!userDao.isPhonenumberUnique(user.getPhonenumber())) {
		ErrrorClazz errorClazz=new ErrrorClazz(1,"PhoneNumber already exists..Please choose another PhoneNumber");	
		return new ResponseEntity<ErrrorClazz>(errorClazz,HttpStatus.INTERNAL_SERVER_ERROR);
	...
	try {
		...
		ErrrorClazz errorClazz=new ErrrorClazz(3,"Unable to register user details"+e.getMessage());	
		return new ResponseEntity<ErrrorClazz>(errorClazz,HttpStatus.INTERNAL_SERVER_ERROR);
}
